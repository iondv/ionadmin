(function ($) {
  ...
  window.listManager = function (model, options) {
    ...
    $tools.find(".edit-btn").click(function () {
      ...
      if (data.length === 1) {
        ...
        messageCallout.warn('Select one object for editing.');
      }
    });
    ...
    $tools.find(".delete-btn").click(function () {
      ...
      if (rows.data().length === 0) {
        messageCallout.warn('Select objects for deletion.');
  ...
  function decorateMaxLinesInCell (data, handler) {
    ...
    if (max < data.length) {
      result += '<span class="" title="Records hidden">... ['+ (data.length - max) +']</span>';
    }
    return result;
  }
