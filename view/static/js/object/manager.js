...
function objectManager (model, fields, options) {
  ...
  function initDropdownObject(field, attr, object) {
    ...
    restApi.getList(options.model).done(function (data) {
      ...
      setFiedlError($field, "Error loading list");
    }).complete(function () {
  ...
  $save.click(function () {
    ...
    ajax.done(function (data) {
      ...
      if (data.errors && data.errors.length) {
        ...
        messageCallout.error("Error saving object");
  ...
  $(window).on("beforeunload", function () {
    ...
    return 'Object not saved. Changes made will be lost.';
  });
  ...
  function validateRequired($field, data) {
    return ($field.length && $field.val().length === 0) ? getValidateMessage(data, "Required field") : null;
  }
  ...
  function validateEmail($field, data) {
    ...
    return reg.test(value) ? null : getValidateMessage(data, "Field value is not an email address");
  }
  ...
  function validateMatch($field, data) {
    var value = $field.val();
    return data.regex.test(value) ? null : getValidateMessage(data, "Field value is wrong");
  }
  ...
  function validateString($field, data) {
    ...
    if (data.min && data.min > length) return "Minimum string length - " + data.min + " vl.";
    if (data.max && data.max < length) return "Maximum line length - " + data.max + " vl.";
    return null;
  }
