...
var restURL = serverURL + '/ionadmin/models/image-list';
var idTest;
var TestImgTitle = 'testEDIT';
var CreateImg = require ('./models-image-create.js').CreateImg;
var EditImg = require ('./models-image-ID.js').EditImg;

describe.skip('# Проверяем доступность страницы Изображения', function () {
  ...
  describe('Проверяем функциональные кнопки', function () {
    it('Проверка загрузки страницы Создать изображение', function (done) {
      CreateImg (done);
      describe('Проверяем Редактирование', function () {
        it('Проверка кнопки Править, правка тестового объекта', function (done) {
          EditImg (done);
          describe('# Проверяем поиск по списку объектов и Удаление изображение', function () {
            ...
            it('Поиск по ID', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(2).getText().then(function (idTest) {
            ...
            it('Удаление нового изображения', function () {
              // Сначала данные сортируются по ID, чтобы наверху оказалась тестовая запись, так как
              // ID выдается в порядке возрастания
