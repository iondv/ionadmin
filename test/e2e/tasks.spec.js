...
var restURL = serverURL + '/ionadmin/tasks';
var testTask = 'New task';

describe.skip('# Проверяем доступность страницы Задачи', function () {
  it ('Ожидаем прогрузку страницы', function () {
    expect(testObj.get(restURL)).to.eventually.equal(restURL);
  });
  describe('# Проверяем загрузку страницы Задачи', function () {
    it('Содержит основные блоки верстки', function () {
      ...
      it('Кнопка добавления задачи', function () {
        element(by.xpath('//*[@id="add-task"]')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Список задач', function () {
        element(by.xpath('//*[@id="task-list"]')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Блок пагинации', function () {
        element(by.xpath('/html/body/div/div/section[2]/div/div[2]/div')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
  ...
  describe('# Проверяем кнопку добавления задачи', function () {
    it('Проверка появления модального окна', function () {
      element(by.xpath('//*[@id="add-task"]')).click();
      browser.sleep(testConfig.browser.eachSleep);
      element(by.id('task-modal')).isDisplayed().then(function (res) {
        expect(res).to.be.true;
      });
    });
    it('Ввод тестовой задачи', function () {
      element(by.xpath('//*[@id="task-description"]')).sendKeys(testTask);
      element(by.xpath('//*[@id="save-task"]')).click();
      element(by.xpath('//*[@id="task-list"]')).all(by.tagName('li')).get(0)
        .element(by.tagName('span')).getText().then(function (res) {
          expect(res).to.be.equal(testTask);
        });
    });
  });
});
