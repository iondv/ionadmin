...
var restURL = serverURL + '/ionadmin/models/postType-list';
var TestPostType = 'test';
var TestPostTypeE = 'edit';
var path = require('path');
var EditPostType = function (done) {
describe.skip('# Проверяем доступность страницы Редактировать тип новости', function () {
  it('Ожидаем прогрузку страницы Редактировать тип новости', function () {
    var idTest;
    testObj.get(restURL);
    ...
    element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
      .get(0).all(by.tagName('td')).get(0).getText().then(function (res) {
        expect(res).to.be.equal(TestPostType);
      });
    element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
    ...
    browser.getAllWindowHandles().then(function (res) {
      ...
      browser.getCurrentUrl().then(function (url) {
        expect(url).to.be.equal(serverURL + '/ionadmin/models/postType/' + idTest);
      });
    });
    describe('Проверяем загрузку страницы Создать муниципальное образование', function () {
      it ('Содержит основные блоки верстки', function () {
        ...
        it('Поле для заполнения кода', function () {
          element(by.xpath('//*[@id="obj-name"]]'))
            .isPresent().then(function (result) {
              expect(result).to.be.true;
            });
        });
        it('Поле для заполнения Названия', function () {
          element(by.xpath('//*[@id="obj-code"]'))
            .isPresent().then(function (result) {
              expect(result).to.be.true;
            });
        });
  ...
  describe('Редактирование Типа новости', function () {
    it('Изменение поля наименование и Сохранение результата', function () {
      element(by.xpath('//*[@id="obj-name"]')).sendKeys(TestPostTypeE);
      element(by.xpath('//*[@id="obj-code"]')).sendKeys(TestPostTypeE);
    ...
    it('Проверка изменений', function () {
      ...
      element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
        .get(0).all(by.tagName('td')).get(0).getText().then(function (res) {
          expect(res).to.be.equal(TestPostType + TestPostTypeE);
        });
      element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
        .get(0).all(by.tagName('td')).get(1).getText().then(function (res) {
          expect(res).to.be.equal(TestPostType + TestPostTypeE);
...
module.exports.EditPostType = EditPostType;
