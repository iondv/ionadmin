...
var EditOff = function (done) {
  describe.skip('Проверяем доступность страницы Редактировать Должн. лицо', function () {
  it('Ожидаем прогрузку страницы Должн. лицо', function () {
    var idTest;
    testObj.get(restURL);
    ...
    element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
      .get(0).all(by.tagName('td')).get(3).getText().then(function (result) {
        idTest = result;
      });
    ...
    browser.getAllWindowHandles().then(function (res) {
      ...
      browser.getCurrentUrl().then (function (url) {
        expect(url).to.be.equal(serverURL + '/ionadmin/models/official/' + idTest);
      });
    });
  });
  ...
  describe.skip('# Изменение всех полей при редактировании', function () {
    ...
    it('Изменение поля Заголовок и Сохранение результата', function () {
      element(by.xpath('//*[@id="obj-sname"]')).sendKeys(TestOffE);
      element(by.xpath('//*[@id="obj-fname"]')).sendKeys(TestOffE);
      element(by.xpath('//*[@id="obj-pname"]')).sendKeys(TestOffE);
      element(by.xpath('//*[@id="obj-position"]')).sendKeys(TestOffE);
    ...
    it('Проверка изменений', function () {
      ...
      element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
        .get(0).all(by.tagName('td')).get(1).getText().then(function (res) {
          expect(res).to.be.equal(TestOff + TestOffE);
        });
    });
  });
});
...
module.exports.EditOff = EditOff;
