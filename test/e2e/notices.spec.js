...
var restURL = serverURL + '/ionadmin/notices';

describe.skip('# Проверяем доступность страницы Уведомления', function () {
  it ('Ожидаем прогрузку страницы', function () {
    expect(testObj.get(restURL)).to.eventually.equal(restURL);
  });
  describe('# Проверяем загрузку страницы Уведомления', function () {
    it('Содержит основные блоки верстки', function () {
      ...
      it('Список уведомлений', function () {
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/ul')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
