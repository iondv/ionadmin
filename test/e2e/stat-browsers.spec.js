// Тестирование меню Статистики броузеров
/**
 * Настройки теста
 * */
...
var restURL = serverURL + '/ionadmin/stat/browsers';

describe.skip('# Проверяем доступность страницы Статистика броузеров', function () {
  ...
  describe('# Проверяем загрузку страницы Просмотр сообщения', function () {
    it('Содержит основные блоки верстки', function () {
      ...
      it('Графический элемент', function () {
        element(by.xpath('/html/body/div/div/section[2]/div/div/div/div[1]')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Список браузеров', function () {
        element(by.xpath('//*[@id="browser-legend"]')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
