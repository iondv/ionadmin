// Тестирование меню Должностные лица
/**
 * Настройки теста
 * */
...
var restURL = serverURL + '/ionadmin/models/official-list';
var idTest;
var TestOff = 'testEDIT';
var CreateOff = require ('./models-official-create.js').CreateOff;
var EditOff = require ('./models-official-ID.js').EditOff;

describe.skip('Проверяем доступность страницы Должностные лица', function () {
  it ('Ожидаем прогрузку страницы', function () {
    expect(testObj.get(restURL)).to.eventually.equal(restURL);
  });
  describe('Проверяем загрузку страницы Должностные лица', function () {
  ...
  describe('Проверяем функциональные кнопки', function () {
    it('Проверка загрузки страницы Создать Должн. лицо', function (done) {
      CreateOff(done);
      describe('Проверяем Редактирование', function () {
        it('Проверка кнопки Править, правка тестового объекта', function (done) {
          ...
          describe('Проверяем поиск по списку объектов и Удаление тестового объекта', function () {
            ...
            it('Поиск по Должности', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(1).getText().then(function (titleTest) {
                  element(by.xpath('//*[@id="object-list_filter"]/label/input')).sendKeys(titleTest);
                  element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                    .count().then(function (result) {
                      expect(result).to.be.at.least(1);
            ...
            it('Поиск по ID', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(3).getText().then(function (idTest) {
            ...
            it('Удаление нового Должностного лица', function () {
              element(by.xpath('//*[@id="object-list"]/thead/tr[2]/th[4]')).click();
              element(by.xpath('//*[@id="object-list"]/thead/tr[2]/th[4]'))
                .getAttribute('aria-sort').then (function (res) {
                if (res == 'ascending') {element(by.xpath('//*[@id="object-list"]/thead/tr[2]/th[4]')).click();}
              });
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(1).getText().then(function (res) {
                  expect(res).to.be.equal(TestOff);
                });
