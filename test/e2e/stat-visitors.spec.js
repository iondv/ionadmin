// Тестирование меню статистики посетителей
/**
 * Настройки теста
 * */
...
var restURL = serverURL + '/ionadmin/stat/visitors';

describe.skip('# Проверяем доступность страницы Статистика посещений', function () {
  it ('Ожидаем прогрузку страницы', function () {
    expect(testObj.get(restURL)).to.eventually.equal(restURL);
  });
  describe('# Проверяем загрузку страницы Cтатистика посещений', function () {
    it('Содержит основные блоки верстки', function () {
      ...
      it('Блок с информацией', function () {
        element(by.xpath('/html/body/div[1]/div/section[2]/div')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
    ...
    it.skip('Все ключевые JS загружаются с кодом 200', function (done) {
      var results = [];
    ...
    it.skip('Все ключевые CSS загружаются с кодом 200', function (done) {
      var results = [];
  ...
  describe('# Проверяем Date range', function () {
    it('Проверка прорисовки элемента Date range', function () {
      element(by.xpath('//*[@id="visitors-daterange"]')).click();
      element(by.xpath('/html/body/div[2]')).isDisplayed().then(function (res) {
        expect(res).to.be.true;
      });
    });
    it.skip('Период за сегодня', function (done) {});
    it.skip('Период за вчера', function (done) {});
    it.skip('Период за последние 7 дней', function (done) {});
    it.skip('Период за последние 30 дней', function (done) {});
    it.skip('Период за текущий месяц', function (done) {});
    it.skip('Период за предыдущий месяц', function (done) {});
    it.skip('Указать период', function (done) {});
  });
});
