...
var TestMunTitle = 'test';
var TestMunTitleE = 'EDIT';
var path = require('path');
var EditMun = function (done) {
  describe.skip('# Проверяем доступность страницы Редактировать муниципальное образование', function () {
  it('Ожидаем прогрузку страницы Редактировать муниципальное образование', function () {
    var idTest;
    testObj.get(restURL);
    ...
    element(by.xpath('//*[@id="object-list"]/thead/tr[2]/th[2]'))
      ...
      if (res == 'ascending') {element(by.xpath('//*[@id="object-list"]/thead/tr[2]/th[2]')).click();}
    });
    ...
    element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
      .get(0).all(by.tagName('td')).get(1).getText().then(function (result) {
        idTest = result;
      });
    ...
    browser.getAllWindowHandles().then(function (res) {
      ...
      browser.getCurrentUrl().then(function (url) {
        expect(url).to.be.equal(serverURL + '/ionadmin/models/municipal/' + idTest);
      });
    });
  ...
  describe.skip('Редактирование Муниципального образования', function () {
    it('Изменение поля наименование и Сохранение результата', function () {
      element(by.xpath('//*[@id="obj-name"]')).sendKeys(TestMunTitleE);
    ...
    it('Проверка изменений', function () {
      ...
      element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
        .get(0).all(by.tagName('td')).get(0).getText().then(function (res) {
          expect(res).to.be.equal(TestMunTitle + TestMunTitleE);
        });
    });
  });
});
...
module.exports.EditMun = EditMun;
