...
var restURL = serverURL + '/ionadmin/models/image/create';
var TestImg = './modules/ionadmin/src/demo-photos/img1.jpg';
var TestImgTitle = 'test';
var CreateImg = function (done) {
  describe.skip('# Проверяем доступность страницы Создать Изображение', function () {
    it('Ожидаем прогрузку страницы', function () {
      expect(testObj.get(restURL)).to.eventually.equal(restURL);
    });
    describe('Проверяем загрузку страницы Создать изображение', function () {
    ...
    describe('Создание тестового изображения', function () {
      ...
      it('Заполнение поля Заголовок и Сохранение результата', function () {
        element(by.xpath('//*[@id="obj-title"]')).sendKeys(TestImgTitle);
        element(by.xpath('//*[@id="save-form"]')).click();
        browser.sleep(testConfig.browser.eachSleep);
