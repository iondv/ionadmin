...
var CreatePost = require ('./models-post-create.js').CreatePost;
var EditPost = require ('./models-post-ID.js').EditPost;
var TestPostTitle = 'teststringedit';

describe.skip('# Проверяем доступность страницы Новости', function () {
  it ('Ожидаем прогрузку страницы', function () {
    expect(testObj.get(restURL)).to.eventually.equal(restURL);
  });
  describe('Проверяем загрузку страницы Изображения', function () {
    it ('Содержит основные блоки верстки', function () {
      ...
      it('Кнопки для работы с изображениями (Обновить, Создать, Править, Удалить)', function () {
  ...
  describe('Проверяем функциональные кнопки', function () {
    it('Проверка загрузки страницы Создать новость, создание новости', function (done) {
      CreatePost(done);
      describe('Проверяем Редактирование', function () {
        it('Проверка кнопки Править, правка тестового объекта', function (done) {
          EditPost(done);
          describe('Проверяем поиск по списку объектов', function () {
            it('Поиск по Дата', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(0).getText().then(function (dataTest) {
                  element(by.xpath('//*[@id="object-list_filter"]/label/input')).sendKeys(dataTest);
                  element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
            ...
            it('Поиск по Тип новости', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(1).getText().then(function (Test) {
                  element(by.xpath('//*[@id="object-list_filter"]/label/input')).sendKeys(Test);
                  element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                    .count().then(function (result) {
                      expect(result).to.be.at.least(1);
                    });
            ...
            it('Поиск по Заголовок', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(2).getText().then(function (Test) {
                  element(by.xpath('//*[@id="object-list_filter"]/label/input')).sendKeys(Test);
                  element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                    .count().then(function (result) {
                      expect(result).to.be.at.least(1);
            ...
            it('Поиск по Публиковать', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(3).getText().then(function (Test) {
                  element(by.xpath('//*[@id="object-list_filter"]/label/input')).sendKeys(Test);
                  element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                    .count().then(function (result) {
                      expect(result).to.be.at.least(1);
            ...
            it('Поиск по Категории', function () {
              element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                .get(0).all(by.tagName('td')).get(5).getText().then(function (Test) {
                  element(by.xpath('//*[@id="object-list_filter"]/label/input')).sendKeys(Test);
                  element(by.xpath('//*[@id="object-list"]/tbody')).all(by.tagName('tr'))
                    .count().then(function (result) {
                      expect(result).to.be.at.least(1);
            ...
            it('Удаление новой новости', function () {
              ...
              al.accept();
              element(by.xpath('//*[@id="object-list_filter"]/label/input')).sendKeys(TestPostTitle);
              element(by.xpath('//*[@id="object-list"]/tbody')).
                element(by.className('dataTables_empty')).isPresent().then(function (res) {
                  expect(res).to.be.true;
                });
