// Тестирование просмотра сообщений
/**
 * Настройки теста
 * */
...
var restURL = serverURL + '/ionadmin/message/view';

describe.skip('# Проверяем доступность страницы Просмотр сообщения', function () {
  it ('Ожидаем прогрузку страницы', function () {
    expect(testObj.get(restURL)).to.eventually.equal(restURL);
  });
  describe('Проверяем загрузку страницы Просмотр сообщения', function () {
    it('Содержит основные блоки верстки', function () {
      ...
      it('Блок с выводом входящих сообщений', function () {
        element(by.xpath('/html/body/div/div/section[2]/div')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Верхние кнопки для работы с сообщениями', function () {
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/div[2]/div/button[1]'))
          .isPresent().then(function (result) {
          expect(result).to.be.true;
        });
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/div[2]/div/button[2]'))
          .isPresent().then(function (result) {
          expect(result).to.be.true;
        });
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/div[2]/div/button[3]'))
          .isPresent().then(function (result) {
          expect(result).to.be.true;
        });
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/div[2]/button'))
          .isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Заголовок сообщения', function () {
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/div[1]/h3'))
          .isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Почтовый адрес отправителя и дата получения письма', function () {
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/div[1]/h5'))
          .isPresent().then(function (result) {
          expect(result).to.be.true;
        });
        element(by.xpath('/html/body/div/div/section[2]/div/div[1]/div[1]/h5/span'))
          .isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Контейнер с текстом сообщения', function () {
        element(by.className('mailbox-read-message')).isPresent().then(function (result) {
          expect(result).to.be.true;
        });
      });
      it('Нижние кнопки для работы с сообщениями', function () {
        element(by.xpath('/html/body/div/div/section[2]/div/div[3]/button[1]'))
          .isPresent().then(function (result) {
            expect(result).to.be.true;
          });
        element(by.xpath('/html/body/div/div/section[2]/div/div[3]/button[2]'))
          .isPresent().then(function (result) {
            expect(result).to.be.true;
          });
        element(by.xpath('/html/body/div/div/section[2]/div/div[3]/div/button[1]'))
          .isPresent().then(function (result) {
            expect(result).to.be.true;
          });
        element(by.xpath('/html/body/div/div/section[2]/div/div[3]/div/button[2]'))
          .isPresent().then(function (result) {
            expect(result).to.be.true;
          });
      });
