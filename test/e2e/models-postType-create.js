...
var restURL = serverURL + '/ionadmin/models/postType/create';
var TestPost = 'test';
var CreatePostType = function (done) {
  describe.skip('# Проверяем доступность страницы Создать тип новости', function () {
  it ('Ожидаем прогрузку страницы', function () {
    expect(testObj.get(restURL)).to.eventually.equal(restURL);
  });
  describe.skip('Проверяем загрузку страницы Создать муниципальное образование', function () {
  ...
  describe.skip('Создание Типа новости', function () {
    ...
    it('Заполнение текстовых полей и Сохранение результата', function () {
      element(by.xpath('//*[@id="obj-code"]')).sendKeys(TestPost);
      element(by.xpath('//*[@id="obj-name"]')).sendKeys(TestPost);
      element(by.xpath('//*[@id="save-form"]')).click();
      browser.sleep(testConfig.browser.eachSleep);
...
module.exports.CreatePostType = CreatePostType;
