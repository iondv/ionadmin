...
var restURL = serverURL + '/ionadmin/api/municipal';
...
var municipalScheme = {
  title: 'Схема данных муниципалитетов',
  type: 'array',
  uniqueItems: true,
  items: {
    title: 'Схема данных объекта муниципалитет',
    type: 'object',
    required: ['id', 'name'],
    properties: {
      id: {
      ...
      name: {
        title: 'Названние муниципалитета',
        type: 'string'
      }
    }
  }
...
describe.skip('# Проверяем API MODEL Municipal', function () {
  describe('Проверяем список по REST запросу: ' + restURL, function () {
    ...
    describe('Проверяем совпадение ответа с моделью', function () {
      it('Проверяем JSON', function () {
        expect(JSON.parse(resBody)).to.be.jsonSchema(municipalScheme);
      });
    });
  });
});
