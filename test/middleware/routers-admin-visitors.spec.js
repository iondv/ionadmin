...
var restURL = serverURL + '/ionadmin/api/stat/visitors';
...
var visitorsScheme = {
  title: 'Схема данных объекта статистики посетителей',
  type: 'object',
  required: ['hosts', 'online', 'countries'],
  properties: {
    hosts: {
      title: 'Хостов',
      type: 'number'
    ...
    countries: {
      ...
      items: {
        title: 'Страны',
        type: 'object',
        required: ['code', 'name', 'hosts', 'online'],
        properties: {
          code: {
            title: 'Код',
            type: 'string'
...
describe.skip('# Проверяем API STAT Visitors', function () {
  describe('Проверяем список по REST запросу: ' + restURL, function () {
    ...
    describe('Проверяем совпадение ответа с моделью', function () {
      it('Проверяем JSON', function () {
        expect(JSON.parse(resBody)).to.be.jsonSchema(visitorsScheme);
      });
    });
  });
});
