...
var restURL = serverURL + '/ionadmin/api/postCategory';
...
var postCategoryScheme = {
  title: 'Схема данных категории новости',
  type: 'array',
  uniqueItems: true,
  items: {
    title: 'Схема данных объекта категории новости',
    type: 'object',
    required: ['image', 'name','_id'],
    properties: {
      _id: {
        title: 'Идентификатор',
        type: ['string', 'number'],
...
describe.skip('# Проверяем API MODEL Post Category', function () {
  describe('Проверяем список по REST запросу: ' + restURL, function () {
    ...
    describe('Проверяем совпадение ответа с моделью', function () {
      it('Проверяем JSON', function () {
        var a = JSON.parse(resBody);
        expect(JSON.parse(resBody)).to.be.jsonSchema(postCategoryScheme);
      });
    });
  });
});
