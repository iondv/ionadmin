...
var restURL = serverURL + '/ionadmin/api/image';
...
describe.skip('# Проверяем API MODEL Image', function () {
  describe('Проверяем список по REST запросу: ' + restURL, function () {
    ...
    describe('Проверяем совпадение ответа с моделью', function () {
      it('Проверяем JSON', function () {
        expect(JSON.parse(resBody)).to.be.jsonSchema(imageScheme);
      });
    });
  });
});
