...
var restURL = serverURL + '/ionadmin/api/stat/models';
...
var objectsScheme = {
  title: 'Схема данных статистики',
  type: 'array',
  uniqueItems: true,
  items: {
    title: 'Схема данных модели статистики',
    type: 'object',
...
describe.skip('# Проверяем API STAT Models', function () {
  describe('Проверяем список по REST запросу: ' + restURL, function () {
    ...
    describe('Проверяем совпадение ответа с моделью', function () {
      it('Проверяем JSON', function () {
        ...
        expect(JSON.parse(resBody)).to.be.jsonSchema(objectsScheme);
      });
    });
  });
});
